...
begin
  ...
  rewrite(mccrpt);
    ...
    on E: EInOutError do begin
      writeln('File handline error occurred. Details: ',E.Message);
    end;
  end;
  writeln(mccrpt);
...
begin
  ...
  for x:=1 to lastrec do begin
    if acountry[x].country<>'' then begin
      write(PadRight(acountry[x].country,55));
      write('     ');
      writeln(acountry[x].attempts);
    end;
  end;
  ReportOut;
end.
